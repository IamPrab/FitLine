...
def post_resultstring(name):  #
    ...
    if (len(values) == 8):
        ...
        X= not_set
        Y= not_set
        if (float(values[0])>1000):
            X = float(values[0])
        elif (float(values[1])>1000):
            X = float(values[1])

        if ((float(values[0])>0 and float(values[0])<2) or (float(values[0])<-500)):
            Y = float(values[0])
        elif ((float(values[1])>0 and float(values[1])<2) or (float(values[1])<-500)):
            Y = float(values[1])
        ...
        if delta<0 and delta>-10 and X==12777.5 and Y==0.63:
            print(delta, slope, intercept, X, Y)
            Y = (slope * X) + intercept
            print(Y)
            print (name)
...
if __name__ == '__main__':
    outPath = "C:\\temp\\axel_temp\\Summary.json"
    outPathEscapes = "C:\\temp\\axel_temp\\Escapes.json"
    outPathEquationStat = "C:\\temp\\axel_temp\\EquationStats.json"
    filePath = "nonnaBerry"

    path = "C:\\temp\\axel_temp\\AuditFullData"
    dir_list = os.listdir(path)


    with open(outPathEscapes, 'w') as filez:

        with open(outPath, 'w') as file2:
            with open(outPathEquationStat, 'w') as file3:
                filez.write("[")
                file2.write("[")
                file3.write("[")
                for file in dir_list:

                    filePath = "C:\\temp\\axel_temp\\AuditFullData\\"+file
                    blocks = read_data(filePath)
                    eqStat = equationStats(blocks)
                    fm_module_cat = cout_FmModuleStatus(blocks)
                    escapes = count_escapes(blocks)

                    reds = red_alert(blocks, filez)


                    result_file2(file2, fm_module_cat, escapes, reds[0], reds[1])


                    resultEquationStat(eqStat[0], eqStat[1], eqStat[2], file3)


                filez.seek(filez.tell() - 1, os.SEEK_SET)
                filez.write("]")
                file2.seek(file2.tell() - 1, os.SEEK_SET)
                file2.write("]")
                file3.seek(file3.tell() - 1, os.SEEK_SET)
                file3.write("]")
